...
{   
    ...
    {
        cout<<"ERROR:packet_handler"<<e.what() <<endl;
        logger.WriteLog("ERROR:packet_handler:"+std::string(e.what()));
    }
...
{   
    try{
        ...
        if(!handle)
        {
            ...
            logger.WriteLog("ERROR:PcapReader::read_pcap:Error opening pcap file:"+ std::string(errbuf));
            return;
        }

        std::cout << "Reading packets from: " << file << std::endl;
        logger.WriteLog("Reading packets from: "+ file);
        ...
        if (pcap_compile(handle, &filter, filter_exp, 0, PCAP_NETMASK_UNKNOWN) == -1)
        {
            ...
            logger.WriteLog("ERROR:PcapReader::read_pcap:Filter compile error");
            return;
        }
        ...
        if (pcap_setfilter(handle, &filter) == -1)
        {
            ...
            logger.WriteLog("ERROR:PcapReader::read_pcap:Filter set error");
            return;
        }
    ...
    {
       cout<<"ERROR:PcapReader::read_pcap"<<e.what() <<endl;
       logger.WriteLog("ERROR:PcapReader::read_pcap:"+std::string(e.what()));
    }
...
void PcapReader::capture_live(const std::string& interface)
{
    ...
    if(!handle)
    {
        ...
        logger.WriteLog("ERROR:PcapReader::capture_live:Error opening interface"+std::string(errbuf));
        return;
    }

    std::cout << "Listening on interface: " << interface << std::endl;
    logger.WriteLog("PcapReader::capture_live:Listening on interface: "+interface);
    ...
    {
        std::cout << "Filter compile error\n";
        logger.WriteLog("PcapReader::capture_live:Filter compile error");
        return;
    }
    ...
    {
        std::cout << "Filter set error\n";
        logger.WriteLog("PcapReader::capture_live:Filter set error ");
        return;
    }
